...
ge1doot.transform3D.Triangle.prototype.draw = function () {
	if (this.p0.visible || this.p1.visible || this.p2.visible) {
		...
		// this.ctx.fillText(this.texture.textContent, 50, 50);
		this.ctx.restore();
	}
	return this.next;
}
/* ===================== image constructor ========================== */
ge1doot.transform3D.Image = function (parent, path, img, lev, callback) {
	this.parent        = parent;
	this.points        = [];
	this.triangles     = [];
	...
	if (img.type === "img") {
		...
		this.texture.src   = path + img.img;
	} else {
		var style_key = "style_earthy_yellow";
		if (StyleLibrary.hasOwnProperty(img.style)) {
			style_key = img.style;
		}
		var textImage = TextImage(StyleLibrary[style_key]);
		this.texture = textImage.toImage(img.text);
	}
	this.isLoading     = true;
